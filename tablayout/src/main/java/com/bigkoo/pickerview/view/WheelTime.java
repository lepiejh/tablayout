...
public class WheelTime {
    ...
    private final boolean arrayMin;

    private boolean isLunarCalendar = false;
    private ISelectTimeCallback mSelectChangeCallback;
    private final List<Integer> minList = new ArrayList<>();

    public WheelTime(View view, boolean[] type, int gravity, int textSize, boolean arrayMin, boolean ymd) {
        super();
        this.view = view;
        this.type = type;
    ...
    private void setLunar(int year, final int month, int day, boolean isLeap, int h, int m, int s) {
        ...
        if (arrayMin) {
            ...
            minList.add(40);
            minList.add(50);
            wv_minutes.setAdapter(new ArrayWheelAdapter<Integer>(minList));
        } else {
    ...
    private void setSolar(int year, final int month, int day, int h, int m, int s) {
        ...
        if (arrayMin) {
            if (year == startYear && (month + 1) == startMonth && day == startDay && h == startHour) {
                ...
                if (m == 1) {
                    ...
                    wv_minutes.setCurrentItem(1);
                } else if (m == 20) {
                    wv_minutes.setCurrentItem(2);
                } else if (m == 30) {
                    wv_minutes.setCurrentItem(3);
                } else if (m == 40) {
                    wv_minutes.setCurrentItem(4);
                } else if (m == 50) {
                    wv_minutes.setCurrentItem(5);
                }
            }
        } else {
            wv_minutes.setAdapter(new NumericWheelAdapter(0, 59));
        ...
        wv_year.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                KLog.d("wheel_time", "年份发生改变 index = " + index);
                int year_num = index + startYear;
        ...
        wv_day.setOnItemSelectedListener(index -> {
            KLog.d("wheel_time", "天数发生改变 index = " + index);
            if (ymd) {
                List<Integer> minList = new ArrayList<>();
                if (currentYear == startYear && currentMonth == startMonth) {
                    if (index == 0) {
                        Calendar cal = TimeNow.Companion.getInstance().getCurrentStarTime();
                        int ho = cal.get(Calendar.HOUR_OF_DAY);
                        wv_hours.setAdapter(new NumericWheelAdapter(ho, 23));
                        if (arrayMin) {
                            int mu = cal.get(Calendar.MINUTE);
                            if (mu == 1) {
                                minList.add(1);
                                minList.add(10);
                                minList.add(20);
        ...
        wv_hours.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                KLog.d("wheel_time", "小时发生改变 index = " + index);
                if (arrayMin) {
                    List<Integer> minList = new ArrayList<>();
        ...
        wv_month.setOnItemSelectedListener(new OnItemSelectedListener() {
            @Override
            public void onItemSelected(int index) {
                KLog.d("wheel_time", "月份发生改变 index = " + index);
...
//                maxItem = endD;
    ...
    public String getTime() {
        ...
        if (arrayMin) {
            ...
            if (size == 5){
                sb.append((wv_minutes.getCurrentItem() + 1) * 10).append(":");
            }else if (size == 4){
                sb.append((wv_minutes.getCurrentItem() + 2) * 10).append(":");
            }else if (size == 3){
                sb.append((wv_minutes.getCurrentItem() + 3) * 10).append(":");
            }else if (size == 2){
                sb.append((wv_minutes.getCurrentItem() + 4) * 10).append(":");
            }else if (size == 1){
                sb.append((wv_minutes.getCurrentItem() + 5) * 10).append(":");
            }else {
                if (wv_minutes.getCurrentItem() == 0) {
                    sb.append("01").append(":");
                } else {
                    sb.append(wv_minutes.getCurrentItem() * 10).append(":");
                }
            }
        } else {
            sb.append(wv_minutes.getCurrentItem()).append(":");
        }
        sb.append(wv_seconds.getCurrentItem());
